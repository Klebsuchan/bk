...
export function generateWhitepaper(lang: 'pt' | 'en') {
  ...
  if (lang === 'pt') {
    ...
    // Capítulo 4
    addHeading("4. Motor de Inteligência Artificial");
    ...
    addParagraph("O usuário apenas digita: 'O funcionário deve ter salário maior que 2000 e ser maior de 18 anos'. O motor LLM interpreta essa intenção e compila automaticamente a regra ZK-SNARK necessária, além das queries SQL de extração do banco de dados (PostgreSQL/MongoDB).");
    ...
    addBullet("A dependência da IA é restrita à criação do circuito e estruturação, não ao processamento dos dados.");
    ...
    addParagraph("The user simply types: 'The employee must have a salary greater than 2000 and be over 18 years old'. The LLM interprets this intent and automatically compiles the necessary ZK-SNARK rule, along with the SQL extraction queries from the database (PostgreSQL/MongoDB).");
    ...
    addBullet("The AI's dependency is restricted to circuit creation and structuring, not data processing.");
